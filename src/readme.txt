...
NEW FILES:
There are several new files that have not been included due to failure to implement them. We have a tac.py file, which contains many labels for different types of machine-independent code. There were also virtual registers, and every TAC label contained one. This was supposed to help us with the liveness map. We had a lot of trouble implementing this correctly and were unable to successfully output code and create a working CFG, or do any optimizations. Thus, it has not been included. This file also included the following functions: labelify, which turned nodes in the AAST to TAC labels; cgen, which converted TAC labels to outputted text, and liveness, which attempted to do a liveness optimization.

We had a codegen.py file as well, which simply served to format and print out the labels properly. It retrieved labels from a global label list and called cgen on them. It also handled all Cool globals and equality handlers.